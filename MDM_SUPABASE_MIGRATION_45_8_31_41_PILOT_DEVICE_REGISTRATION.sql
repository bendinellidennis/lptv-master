...
-- BUILD 45.8.31.41 - PILOT ACCESS & LICENSING GATE / DEVICE REGISTRATION
-- Run AFTER 45.8.31.40 License Seat Assignment has been installed and functionally verified.
-- This step registers an authenticated user's device against an ACTIVE license seat.
-- It does NOT yet enforce the device_limit. Limit enforcement is the NEXT step.
...
begin
  ...
  if found then
    ...
    return jsonb_build_object(
      ...
      'device_limit_enforced', false,
      'next_step', 'device_limit_enforcement'
    );
  end if;
