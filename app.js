...
async function refreshApplication(){
 ...
 try{
  ...
  if(window.caches){
   ...
   await Promise.all(keys.filter(key=>key!=='mdm-build-24-1').map(key=>caches.delete(key)));
  }
 }catch{}
...
function aiInstructorViewHtml(){
 ...
 return [
  `<div class="section-title"><div><h2>🧠 ${esc(t('aiInstructor'))}</h2><p>${esc(t('aiInstructorSub'))}</p></div><span class="badge official">Build 24</span></div>`,
  `<div class="ai-offline-banner"><span>🔒</span><div><strong>${esc(t('aiInstructorOffline'))}</strong><p>${esc(t('aiInstructorOfflineText'))}</p></div></div>`,
...
function showExplanation(q,a){
 ...
 ${errorReasonHtml(q,a)}
 <div class="ai-context-launch"><button class="btn" id="quizAiInstructor">🧠 ${esc(t('aiInstructorAsk'))}</button><button class="btn secondary" id="quizAiUnderstand">🧩 ${esc(t('aiInstructorUnderstand'))}</button></div>`;
...
async function exportBackup(){
 const data={app:'Malta Driving Master',version:'24.1',exportedAt:new Date().toISOString(),progress,settings,userProfile,schoolPreferences,schoolCompare,schoolPartnerDraft,schoolDashboard,schoolPortal,instructorPortal,aiInstructor,lptvPassport,personalRoadmap,onboarding,privacyPreferences,coachState,recoveryState,examDayState,zeroErrorState};
