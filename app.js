...
async function refreshApplication(){
 ...
 try{
  ...
  if(window.caches){
   ...
   await Promise.all(keys.filter(key=>key!=='mdm-build-20').map(key=>caches.delete(key)));
  }
 }catch{}
...
async function exportBackup(){
 const data={app:'Malta Driving Master',version:'20.0',exportedAt:new Date().toISOString(),progress,settings,userProfile,schoolPreferences,schoolCompare,schoolPartnerDraft,schoolDashboard,lptvPassport,personalRoadmap,onboarding,privacyPreferences,coachState,recoveryState,examDayState};
