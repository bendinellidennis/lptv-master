...
async function refreshApplication(){
 ...
 try{
  ...
  if(window.caches){
   ...
   await Promise.all(keys.filter(key=>key!=='mdm-build-22').map(key=>caches.delete(key)));
  }
 }catch{}
...
function schoolPortalViewHtml(){
 ...
 return [
  `<div class="section-title"><div><h2>🏫 ${esc(t('schoolPortal2'))}</h2><p>${esc(t('schoolPortal2Sub'))}</p></div><span class="badge partner-badge">Build 22</span></div>`,
...
async function exportBackup(){
 const data={app:'Malta Driving Master',version:'22.0',exportedAt:new Date().toISOString(),progress,settings,userProfile,schoolPreferences,schoolCompare,schoolPartnerDraft,schoolDashboard,schoolPortal,lptvPassport,personalRoadmap,onboarding,privacyPreferences,coachState,recoveryState,examDayState,zeroErrorState};
