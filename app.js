...
async function refreshApplication(){
 ...
 try{
  ...
  if(window.caches){
   ...
   await Promise.all(keys.filter(key=>key!=='mdm-build-25').map(key=>caches.delete(key)));
  }
 }catch{}
...
function cloudReadyViewHtml(){
 ...
 return [
  `<div class="section-title"><div><h2>☁️ ${esc(t('cloudReady'))}</h2><p>${esc(t('cloudReadySub'))}</p></div><span class="badge official">Build 25</span></div>`,
...
function missionViewHtml(){
 ...
 return [
  `<div class="section-title"><div><h2>🎯 ${esc(t('missions'))}</h2><p>${esc(t('missionsSub'))}</p></div><span class="badge official">Build 25</span></div>`,
...
async function exportBackup(){
 const data={app:'Malta Driving Master',version:'25.0',exportedAt:new Date().toISOString(),progress,settings,userProfile,schoolPreferences,schoolCompare,schoolPartnerDraft,schoolDashboard,schoolPortal,instructorPortal,aiInstructor,cloudReady,missionSystem,lptvPassport,personalRoadmap,onboarding,privacyPreferences,coachState,recoveryState,examDayState,zeroErrorState};
