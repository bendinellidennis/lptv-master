...
let progress = load(STORAGE,{seen:{},correct:{},wrong:{},exams:[],favourites:[],activity:{},knownWords:[],review:{}});
...
function bindViewSpecific(){
 ...
 if(route.name==='profile'){
   ...
   $('#clearProgress').onclick=()=>{if(confirm(t('resetConfirm'))){progress={seen:{},correct:{},wrong:{},exams:[],favourites:[],activity:{},knownWords:[],review:{},bankVersion:TAG_BANK_VERSION};save(STORAGE,progress);localStorage.removeItem(SESSION);render()}}
 }
...
function confirmAnswer(){const q=quiz.list[quiz.index],need=q.correct.length;if(quiz.selected.length!==need)return toast(need===1?t('selectOne'):t('selectMany',need));const chosen=[...quiz.selected].sort((a,b)=>a-b),correct=[...q.correct].sort((a,b)=>a-b),ok=JSON.stringify(chosen)===JSON.stringify(correct);quiz.answers[q.id]={selected:chosen,ok};quiz.answered=true;progress.seen[q.id]=(progress.seen[q.id]||0)+1;if(ok)progress.correct[q.id]=(progress.correct[q.id]||0)+1;else progress.wrong[q.id]=(progress.wrong[q.id]||0)+1;recordActivity();updateReviewSchedule(q,ok);save(STORAGE,progress);saveSession();if(quiz.mode==='exam')nextQuestion();else renderQuiz()}
...
async function exportBackup(){
 const data={app:'Malta Driving Master',version:'7.0',exportedAt:new Date().toISOString(),progress,settings};
