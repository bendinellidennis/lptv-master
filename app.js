...
async function refreshApplication(){
 ...
 try{
  ...
  if(window.caches){
   ...
   await Promise.all(keys.filter(key=>key!=='mdm-build-19-1').map(key=>caches.delete(key)));
  }
 }catch{}
...
async function exportBackup(){
 const data={app:'Malta Driving Master',version:'19.1',exportedAt:new Date().toISOString(),progress,settings,userProfile,schoolPreferences,schoolCompare,schoolPartnerDraft,schoolDashboard,lptvPassport,personalRoadmap,onboarding,privacyPreferences,coachState,recoveryState};
