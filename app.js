...
function updateChrome(){document.documentElement.lang=settings.lang;$('#langBtn').textContent=settings.lang.toUpperCase();document.querySelectorAll('[data-i18n]').forEach(x=>x.textContent=t(x.dataset.i18n));backBtn.classList.toggle('hidden',route.name==='home');$('#bottomNav').classList.toggle('hidden',route.name==='quiz');document.querySelectorAll('[data-nav]').forEach(x=>x.classList.toggle('active',x.dataset.nav===route.name));}
...
function render(){updateChrome();const fn=views[route.name]||views.home;screen.innerHTML=fn(route.data);screen.focus({preventScroll:true});window.scrollTo(0,0);bindCommon();if(route.name==='quiz')renderQuiz();if(route.name==='assistant')bindAssistant();}
...
const views={
 ...
 quiz:()=>`<div class="card quiz-card"><div class="quiz-head"><span class="badge counter-badge" id="qCounter"></span><span class="timer" id="quizTimer"></span></div><div class="progress"><span id="quizProgress"></span></div><div class="quiz-mode-row"><span class="mode-label" id="quizModeBadge"></span><span id="quizMeta" class="muted small"></span></div><div id="quizQuestion" class="question"></div><div id="quizHelp"></div><div id="quizInstruction" class="quiz-instruction"></div><div id="quizOptions"></div><div id="quizExplanation" class="explanation hidden"></div><div class="actions quiz-footer"><button class="btn secondary" id="quizExit">${esc(t('exit'))}</button><button class="btn" id="quizConfirm">${esc(t('confirm'))}</button><button class="btn hidden" id="quizNext">${esc(t('next'))}</button></div></div>`,
 result:()=>{const r=route.data;return `<div class="card" style="text-align:center"><span class="badge ${r.pass?'official':'warning'}">${esc(r.title)}</span><div class="score">${r.correct}/${r.total}</div><p class="muted">${Math.round(r.correct/r.total*100)}%</p><div class="actions"><button class="btn secondary" data-go="lptv">${esc(t('close'))}</button>${r.wrongIds.length?`<button class="btn" id="reviewWrong">${esc(t('resultReview'))}</button>`:''}</div></div>`}
};

function bindViewSpecific(){
 if(route.name==='lptv'){
   $('#examBtn').onclick=()=>startQuiz(buildExam(),'exam',{timerSeconds:2700});
 ...
 if(route.name==='studysetup')$('#startStudy').onclick=()=>{const cat=$('#studyCategory').value,n=Number($('#studyCount').value);const pool=cat==='ALL'?Q:Q.filter(q=>q.category===cat);startQuiz(shuffle(pool).slice(0,Math.min(n,pool.length)),'guided')};
...
function buildExam(){const official=Q.filter(q=>['official_transport_malta','verified','school_material_confirmed'].includes(q.status));return shuffle(official.length>=35?official:Q).slice(0,35)}
function startQuiz(list,mode,options={}){
 const timerSeconds=Number(options.timerSeconds||0);
 quiz={list,index:0,mode,answers:{},selected:[],remaining:mode==='exam'?2700:timerSeconds,timerEnabled:mode==='exam'||timerSeconds>0,answered:false,showTranslation:false};
...
function startTimer(){clearInterval(timerId);timerId=setInterval(()=>{quiz.remaining=Math.max(0,quiz.remaining-1);const el=$('#quizTimer');if(el){el.textContent=formatTime(quiz.remaining);el.classList.toggle('danger',quiz.remaining<=300)}saveSession();if(quiz.remaining<=0)finishQuiz()},1000)}
...
function renderQuiz(){
  ...
  $('#quizMeta').textContent=`${q.category} • ${q.status||'verified'} • ${q.id}`;
  $('#quizQuestion').textContent=q.question;
  $('#quizInstruction').textContent=q.correct.length===1?t('selectOne'):t('selectMany',q.correct.length);

  const help=$('#quizHelp');
  if(quiz.mode==='exam'){
    help.innerHTML=`<div class="exam-lock">🔒 ${esc(t('noHelpExam'))}</div>`;
  }else{
  ...
  $('#quizConfirm').classList.toggle('hidden',!!quiz.answered);
  $('#quizNext').classList.toggle('hidden',!quiz.answered);
  screen.querySelectorAll('[data-opt]').forEach(b=>{b.onclick=()=>selectOption(Number(b.dataset.opt))});
  $('#quizExit').onclick=()=>{if(confirm(t('exit')+'?')){localStorage.removeItem(SESSION);go('lptv')}};
...
function selectOption(i){if(quiz.answered)return;const q=quiz.list[quiz.index],max=q.correct.length;if(quiz.selected.includes(i))quiz.selected=quiz.selected.filter(x=>x!==i);else{if(quiz.selected.length>=max)return toast(max===1?t('selectOne'):t('selectMany',max));quiz.selected.push(i)}saveSession();renderQuiz()}
...
function nextQuestion(){if(quiz.index<quiz.list.length-1){quiz.index++;quiz.selected=[];quiz.answered=false;quiz.showTranslation=false;saveSession();renderQuiz();window.scrollTo(0,0)}else finishQuiz()}
function finishQuiz(){clearInterval(timerId);timerId=null;const vals=Object.values(quiz.answers),correct=vals.filter(x=>x.ok).length,total=quiz.list.length,wrongIds=quiz.list.filter(q=>!quiz.answers[q.id]?.ok).map(q=>q.id),pass=quiz.mode==='exam'&&correct>=30;if(quiz.mode==='exam'){progress.exams.push({date:new Date().toISOString(),score:correct,total});progress.exams=progress.exams.slice(-30);save(STORAGE,progress)}localStorage.removeItem(SESSION);const title=quiz.mode==='exam'?(pass?t('passed'):t('failed')):t('completed');quiz=null;go('result',{correct,total,pass,title,wrongIds})}
...
function stats(){const seen=Object.keys(progress.seen||{}).length,attempts=Object.values(progress.seen||{}).reduce((a,b)=>a+b,0),correct=Object.values(progress.correct||{}).reduce((a,b)=>a+b,0),exams=progress.exams||[],scores=exams.map(e=>e.score);return{seen,accuracy:attempts?Math.round(correct/attempts*100):0,exams:exams.length,best:scores.length?Math.max(...scores)+'/35':null,last:scores.length?scores.at(-1)+'/35':null}}
...
function renderVocabulary(term){
 ...
 box.querySelectorAll('[data-known-word]').forEach(b=>b.onclick=()=>{toggleKnownWord(b.dataset.knownWord);renderVocabulary(input.value)});
}
