/* Malta Driving Master 45.8.36 — Compact Home */
(function(){
'use strict';
if(window.MDM_COMPACT_HOME_45836)return;
const V='45.8.36', HUB='mdmCompactRealPreparation';
let open='', raf=0;
...
function passportText(){
 ...
 return code+' · '+verified+' '+t('verificate','verified','ivverifikati')+' · '+checking+' '+t('in verifica','in verification','qed jiġu vverifikati');
}
function rows(){
 return [
...
function setVisibility(){
 const c=cards();
 if(c.proof)c.proof.hidden=!(open==='proof'||open==='mission');
 if(c.exam)c.exam.hidden=open!=='exam';
 if(c.passport)c.passport.hidden=open!=='passport';
 if(c.proof&&!c.proof.hidden)collapseButton(c.proof,open);
 if(c.exam&&!c.exam.hidden)collapseButton(c.exam,'exam');
 if(c.passport&&!c.passport.hidden)collapseButton(c.passport,'passport');
}
function renderHub(){
 ...
 hub.innerHTML='<header><div><small>MDM PROOFLOOP · '+V+'</small><h2>'+esc(t('La tua preparazione reale','Your real preparation','Il-preparazzjoni reali tiegħek'))+'</h2><p>'+esc(t('Apri solo ciò che ti serve.','Open only what you need.','Iftaħ biss dak li għandek bżonn.'))+'</p></div><span>⚡</span></header><div class="list">'+body+'</div>';
...
function apply(){
 if(!home()){
  ...
  Object.values(cards()).filter(Boolean).forEach(x=>x.hidden=false);
  return;
 }
 if(!renderHub())return;
 setVisibility();
