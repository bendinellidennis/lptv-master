/* Malta Driving Master 45.8.35 — Driver Competence Passport */
(function(){
'use strict';
if(window.MDM_DRIVER_COMPETENCE_PASSPORT)return;

const VERSION='45.8.35';
...
function load(){
 ...
 return v&&v.schema==='mdm-driver-competence-passport-v1'?v:{schema:'mdm-driver-competence-passport-v1',version:VERSION,packs:{},history:[],createdAt:now(),updatedAt:now()};
}
function save(v){v.version=VERSION;v.updatedAt=now();localStorage.setItem(key(),JSON.stringify(v));return v}
...
function appendEvent(pack,type,id,data){
 if(pack.events.some(e=>e.type===type&&e.id===id))return;
...
function render(){
 ...
 const state=sync();if(!viewPack)viewPack=state.activePackId;
 if(!PACKS.some(x=>x.id===viewPack))viewPack=state.activePackId;
 ...
 if(!anchor)return;
 let card=old;if(!card){card=document.createElement('section');card.id='mdmDriverCompetencePassport';card.className='mdm-competence-passport';anchor.insertAdjacentElement('afterend',card)}
