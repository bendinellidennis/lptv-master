...
/* 45.8.31.49.18 — deterministic Pilot/account bridge loader.
   School Home alert/direct queue is owned ONLY by mdm-pwa-refresh-4583147.js.
   Removed the duplicate legacy Home alert handler that still redirected to Profile.
...
window.addEventListener('load',function(){
  try{
    ...
    if(!window.MDM_PWA_REFRESH_FIX){const refresh=document.createElement('script');refresh.src='mdm-pwa-refresh-4583147.js?v=458314917-direct-modal';refresh.async=true;refresh.setAttribute('data-mdm-pwa-refresh-fix','direct-school-queue');document.head.appendChild(refresh);}
    ...
    async function renderSeatQueue(){
      const host=findSchoolConsole(),old=document.getElementById('mdmPilotSeatAssignPanel');
      ...
      try{
        ...
        let rows=await pilotRpc('mdm_school_list_redeemed_pilot_invitations',{});if(!Array.isArray(rows))rows=rows?[rows]:[];
        ...
        rows.forEach(row=>{
          ...
          const email=String(row.invite_email||'Studente'),id=String(row.invitation_id||'');
        ...
        return true;
      }catch(e){let panel=document.getElementById('mdmPilotSeatAssignPanel');if(panel)panel.innerHTML='<strong>🎟️ Studenti da attivare</strong><div style="margin-top:8px;font-size:12px">❌ '+String(e?.message||e||'seat_queue_failed')+'</div>';return false;}finally{seatLoading=false;}
    }
