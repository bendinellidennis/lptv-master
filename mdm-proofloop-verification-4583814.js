...
function bindCosign(local,rerender){
 ...
 if(show&&area)show.onclick=function(){
  area.value=cosignEncode(local);
  area.hidden=false;
  show.textContent='📋 '+t('Codice pronto','Code ready','Kodiċi lest');
 };

 if(check)check.onclick=async function(){
  check.disabled=true;
  check.textContent='⏳ '+t('Controllo…','Checking…','Qed jiġi ċċekkjat…');
