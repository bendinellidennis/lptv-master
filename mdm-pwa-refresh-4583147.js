/* Malta Driving Master 45.8.31.49.16 — immediate Home School request alert.
   The Home alert appears while the real school queue is being checked, instead of waiting for Profile.
   If the backend returns no pending students or the account is not authorized, the alert is removed.
   If students are pending, it becomes the exact count and one tap opens the seat queue.
   SHADOW / enforcement unchanged. */
(function(){
  'use strict';
  const RELEASE='45.8.31.49.16';
  ...
  function ensureAlert(){const app=document.getElementById('app'),screen=document.getElementById('screen');if(!app||!screen)return null;let el=document.getElementById('mdmSchoolActivationAlert');if(!el){el=document.createElement('button');el.id='mdmSchoolActivationAlert';el.type='button';el.style.cssText='display:flex;width:calc(100% - 24px);margin:8px 12px 4px;box-sizing:border-box;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border:1px solid rgba(245,158,11,.40);border-radius:14px;background:#fff7df;color:#3f2b00;font:inherit;text-align:left;box-shadow:0 8px 24px rgba(3,25,38,.12);cursor:pointer;z-index:35';el.onclick=jump;app.insertBefore(el,screen);}return el;}
  ...
  function showAlert(rows){if(!rows.length){removeAlert();return;}const el=ensureAlert();if(!el)return;const n=rows.length;el.innerHTML=`<span><strong>🔔 ${n} ${esc(n===1?lang3('studente da attivare','student to activate','student biex jiġi attivat'):lang3('studenti da attivare','students to activate','studenti biex jiġu attivati'))}</strong><small style="display:block;margin-top:3px;opacity:.72">${esc(lang3('Tocca per aprire direttamente la richiesta','Tap to open the request directly','Agħfas biex tiftaħ it-talba direttament'))}</small></span><span style="font-size:22px;font-weight:900">›</span>`;}
  ...
  async function refreshAlert(){
    ...
    try{let rows=await rpc('mdm_school_list_redeemed_pilot_invitations',{});clearTimeout(pendingTimer);if(seq!==alertRequestSeq)return;if(!Array.isArray(rows))rows=rows?[rows]:[];showAlert(rows);}catch(_){clearTimeout(pendingTimer);if(seq===alertRequestSeq)removeAlert();}
  }
